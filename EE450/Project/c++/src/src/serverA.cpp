...
void printSolution(vector<int> dist, map<int, int> map2, MinPath *result){
	char dash[50] = "-------------------------------------------------";

	int size = dist.size();

	vector<int> vertex(size, 0);
...
void pathFinding(char city, int startNode, list<City> *clist, MinPath *result){
	...
	printSolution(dist, map2, result);
...
int main()
{
	...
	while(1){
		...
		//send back to aws
		sendto(sockfd, retVal, sizeof retVal , 0,(struct sockaddr *) &their_addr, addr_len);		

		int size = result->vertex.size();
		sendto(sockfd, &size, sizeof size , 0,(struct sockaddr *) &their_addr, addr_len);
		...
		sendto(sockfd, vertex, sizeof vertex , 0,(struct sockaddr *) &their_addr, addr_len);
		sendto(sockfd, minlength, sizeof minlength , 0,(struct sockaddr *) &their_addr, addr_len);
