...
int main()
{
	...
	while(1){
		...
		if((numbytes = recvfrom(sockfd, msg, sizeof msg , 0,(struct sockaddr *)&their_addr, &addr_len)) == -1){
			perror("serverB: failed to recv data from AWS server");
			exit(1);
		}

		long long int fileSize = 0;
		if((numbytes = recvfrom(sockfd, &fileSize, sizeof fileSize , 0,(struct sockaddr *)&their_addr, &addr_len)) == -1){
			perror("serverB: failed to recv file size from AWS server");
			exit(1);
		}		
		// printf("filesize: %llu\n", fileSize);
		...
		if((numbytes = recvfrom(sockfd, vertex, sizeof vertex , 0,(struct sockaddr *)&their_addr, &addr_len)) == -1){
			perror("serverB: failed to recv vertex array from AWS server");
			exit(1);
		}	
		if((numbytes = recvfrom(sockfd, minlength, sizeof minlength , 0,(struct sockaddr *)&their_addr, &addr_len)) == -1){
			perror("serverB: failed to recv minlength array from AWS server");
			exit(1);
		}			
    	...
    	if((numbytes = sendto(sockfd, result, sizeof result , 0,(struct sockaddr *) &their_addr, addr_len)) == -1){
    		perror("serverB: failed to send data to AWS server");
    		exit(1);
    	}		
