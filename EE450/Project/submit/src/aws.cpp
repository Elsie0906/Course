...
int createUDPSocket(){
	...
	for(p = servinfo; p != NULL; p = p->ai_next){
		...
		if (bind(AWSSocket_UDP, p->ai_addr, p->ai_addrlen) == -1){
			close(AWSSocket_UDP);
			perror("aws usdp: bind");
			continue;
		}		
		break;
	}
	...
	// int getsock_check=getsockname(AWSSocket_UDP,(struct sockaddr*)&my_addr, &addrlen);
	// //Error checking
	// if (getsock_check== -1) { 
	// 	perror("getsockname"); 
	// 	exit(1);
	// }
	// else{
	// 	printf("port number %d, line: %d\n", ntohs(my_addr.sin_port), __LINE__);
	// }     
...
int talkWithServerA(int AWSSocket_UDP,char mapID, int startNode, MinPath *result){	
    ...
    if((numbytes = sendto(AWSSocket_UDP, &mapID, sizeof mapID, 0, (struct sockaddr*)&addrServerA, addrServerA_length)) == -1){
    	perror("talkerA: sendto");
    	exit(1);
    }

    if((numbytes = sendto(AWSSocket_UDP, &startNode, sizeof startNode, 0, (struct sockaddr*)&addrServerA, addrServerA_length)) == -1){
    	perror("talkerA: sendto");
    	exit(1);
    }
...
int talkWithServerB(int AWSSocket_UDP,MinPath *buf, long long int fileSize){	
...
int main()
{
	...
	while(1){
		...
		printf("The AWS has received map ID %c, start vertex %d and file size %llu from the client using TCP over port %s\n", mapID, srcNode, fileSize, AWSPORTTCP);
		
		MinPath *result = new MinPath();
		// configure the socket for the connection aws-serverA
		// Send the data to serverA and receive the output from it
		talkWithServerA(udp, mapID, srcNode, result);
		// configure the socket for the connection aws-serverB
