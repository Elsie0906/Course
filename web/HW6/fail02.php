<?php if(isset($_REQUEST["q"])): ?>
    <?php 
        ...
        //echo $addr.'&'.$loc.'&'.$radius.'&'.$type.'&'.$keyword.'<br>';
...
<script type="text/javascript">
    ...
    ori_lng = "";
    whatMap = "";
    function requestGeoLoc(address){

        var param = encodeURI(address); 
        console.log(param);

        var xhttp = new XMLHttpRequest();

        xhttp.open("GET", "place.php?y=" + param, false);
        xhttp.send();

        //console.log(this.responseText);
        
        var word = this.responseText.split(",");    
    ...
    function clearMap(){
        ...
        for(var i=0; i<global_id.length; i++){
            ...
            if( x != null && x.hasAttribute("hidden") == false){
                x.setAttribute("hidden",true);
            }
        }
    ...
    function showMeResult(){   
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {   
                ...
                if( document.myForm.location.value == "Here"){
                    ori_lat = document.myForm.lat.value;
                    ori_lng = document.myForm.lon.value;               
                }
                else{
                //var address = encodeURI(document.myForm.in_loc.value);
                    requestGeoLoc(document.myForm.in_loc.value);
                }        
...
<?php endif; ?> 
    ...
    <?php 
        ...
        function nearbySearch($location,$radius,$type,$keyword){
            ...
            $str = urlencode($type);
            $url = $prefix.$location."&".$radius."&".$str."&".$keyword."&key=".$myKey;
