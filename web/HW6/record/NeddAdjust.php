...
<?php elseif(isset($_REQUEST["p"])): ?>
    <?php
        ...
        echo getPhoto($name,$width,$reference);
    ?>     
<?php else: ?>
...
<script type="text/javascript">
    ...
    function requestPhoto(num,reference,width,height){
        ...
        xhttp.open("GET", "place.php?p=" + param, true);
        xhttp.send();        
    ...
    function getPhoto(jsonObj){
        ...
        if( jsonObj.status != "ZERO_RESULTS"){
            ...
            for(var i=0; i<photoNo; i++){
                ...
                var file = "Img" + i + ".png";
                text += "<img src='"+ file +"' alt='" + file +"' width='240px'>";
                //text += "<img src='Img0.png' alt='Img0.png' width='240px'>";
    ...
    function showDetail(review, title, photo){
        var text = "";

        text += "<h style='font-size:16px;'>"+ title +"</h>";

        text += "<div style='padding-top: 10px'>"
        text += "<p class='Arrow'>click to show reviews</p>";
        text += "<p class='DownArrow'>&#x2304;</p>";
        text += review;
        text += "<p class='Arrow'>click to show photos</p>";
        text += "<p class='DownArrow'>&#x2304;</p>";
        text += photo;
        text += "</div>";
    ...
    function getDetail(where, title){
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) { 
                ...
                var photo = getPhoto(jsonObj);

                //console.log(photo);

                var text = showDetail(review, title,photo);
