...
public class FindStarbucks {
	...
	public static void preprocess(String vidFileName, String logoFileName) {
		...
		openLogo(logoFile);
		readFile(0, logoRand, logoH, logoS, logoV); // HSV color space
		makeHistogramForLogo(logoH, logoS, logoV, logoHHist, logoWBHist); // HSV color histogram; hue is considdr to be the most important
		// keepTopValsOfHistogram(logoHHist, logoWBHist); // keep first 4-value; get rid of noise(????)
		TOTAL_PIXELS_LOGO = calcTotalPixels(logoHHist, logoWBHist);
		TOTAL_COLORS_LOGO = calcTotalColors(logoHHist, logoWBHist);
		proportionalizeHistogram(logoHHist, logoWBHist, logoProportionsHHist, logoProportionsWBHist, TOTAL_PIXELS_LOGO); // normalized
		try {
			logoRand.close();
		} catch (IOException e) {
	...
	//public static void findLogo(VideoFrame frameObj, int[][] H, int[][] S, int[][] V) 
	public static void findLogo(VideoFrame frameObj){
		...
		for (int y = 45; y < (HEIGHT - 45 - GRID_SIZE); y += 10) {
			for (int x = 45; x < (WIDTH - 45 - GRID_SIZE); x += 10) {
				...
				temp = getQuotientHist(vidHHist, vidWBHist, quotientHHist, quotientWBHist, vidProportionsHHist, vidWBProportionsHist); // find similarity between two hist (entropy)
		...
		if (logoConfidence > 0.85){ 
			// System.out.println("Finding Starbucks: " + logoConfidence);
	...
	private static void proportionalizeHistogram(int[] hHist, int[] wbHist, float[] proportionsH, float[] proportionsWb, int totalPixels) {
		for (int i = 0; i < BANDS; i++) {
	...
	private static float getQuotientHist(int[] hHist, int[] wbHist, float[] quoHHist, float[] quoWbHist, float[] proportionsH, float[] proportionsWb) {
		int newPixelCount, colorCount = 0, totalCountGrid = 0, gridBasedLogoPixels = 0;
		...
		// check needed value; for example, logo has feature "Green", we should check green pixel in the grid
		...
		if (colorCount > Math.round(0.5 * TOTAL_COLORS_LOGO) && (totalCountGrid > 2430)) {
			...
			// gridBasedLogoPixels = calcTotalPixels(logoHHist, logoWBHist);
			// entropyLogo = calcEntropy(logoHHist, logoWBHist, gridBasedLogoPixels);
